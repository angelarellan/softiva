...
export default async function Services() {
  ...
  const { eyebrow, titlePre, titleHighlight, verPlanes, verTodos } = dict.home.servicesSection;
    ...
    <section className="relative pt-0 pb-10 md:py-12 [content-visibility:auto] [contain-intrinsic-size:auto_1100px]">
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {categories.map(({ slug, icon: Icon, title, summary }, index) => (
            <Reveal key={slug} delay={index * 0.08} className="h-full">
              <Link
                href={localeHref(locale, `/servicios#${slug}`)}
